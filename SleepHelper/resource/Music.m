...
+(void)load{
    ...
    //初始化音效播放器
    NSString *effective_path = [[NSBundle mainBundle]pathForResource:@"1876" ofType:@"wav"];
    ...
    _instance.musiceffective.volume = 3;
    ...
    _instance.mergeEffectice.volume = 1;
//    _instance.mergeEffectice.enableRate = YES;
//    _instance.mergeEffectice.rate = 1.6;
...
-(void)musicChangeMute{
    if (_instance.BGM.volume > 0.0 ) {
        ...
        _instance.mergeEffectice.volume = 0;
    }else{
        _instance.BGM.volume = 0.5;
        _instance.musiceffective.volume = 3;
        _instance.mergeEffectice.volume = 1;
